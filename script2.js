// Script para enviar el guion de Shrek línea por línea en WhatsApp Web
// Para detener la ejecución, escribe en la consola: detenerEnvio = true

console.log("Cargando guion de Shrek...");
...
// Dividir el guion en líneas (similar al split original)
const lineas = guionShrek.split(/[\n\t]+/).map(line => line.trim()).filter(line => line.length > 0);
console.log(`Guion cargado. Total de líneas: ${lineas.length}`);

// Variable global para detener el envío
window.detenerEnvio = false;
...
async function enviarMensaje(texto) {
    ...
    if (boton.disabled) {
        console.error("Botón deshabilitado");
        return false;
    }

    boton.click();
    console.log(`✅ Enviado: ${texto.substring(0, 50)}...`); // Mostrar solo inicio
    await sleep(500);
    return true;
}

// Función para enviar todas las líneas
async function enviarGuion() {
    console.log("Iniciando envío del guion de Shrek...");
    console.log("Para detener, escribe en la consola: detenerEnvio = true");
    ...
    for (let i = 0; i < lineas.length; i++) {
        ...
        console.log(`Progreso: ${i+1}/${lineas.length} - Enviando línea...`);
        const exito = await enviarMensaje(lineas[i]);
        if (!exito) {
            console.error("Error al enviar. Deteniendo.");
            break;
        }
...
// Ejecutar
enviarGuion().catch(err => console.error("Error general:", err));
